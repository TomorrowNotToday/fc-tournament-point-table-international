...
def get_groups():
    ...
    for g in groups:
        ...
        for m in matches:
            ...
            # --- Normalize legacy 'status' field to 'isLive'/'isFinished' booleans ---
            # Old DB schema uses: status = "scheduled" | "live" | "finished"
            # New code expects: isLive (bool), isFinished (bool)
            if 'status' in m:
                status = m['status']
                if 'isLive' not in m:
            ...
            # Also ensure defaults if neither field exists
            if 'isLive' not in m:
                m['isLive'] = False
            if 'isFinished' not in m:
                m['isFinished'] = False
        
        # Calculate standings dynamically from non-hidden matches
        standings = {}
        for team in g['teams']:
            standings[team['id']] = {
                'id': team['id'],
        ...
        # Sort standings: PTS -> GD -> GF -> Alphabetical Name
...
def start_match(current_user):
    ...
    # Accept both new keys and legacy keys in case of frontend caching
    ...
    home_team = next((t for t in group['teams'] if t['id'] == home_team_id), None)
    away_team = next((t for t in group['teams'] if t['id'] == away_team_id), None)

    if not home_team or not away_team:
        return jsonify({'message': 'Teams not found in group'}), 404

    # --- Duplicate & Resume match logic ---
    # 1. First, check if there is ALREADY a finished match for this exact matchup
    ...
    if existing_finished:
        return jsonify({
            'message': f"Pertandingan {home_team['name']} (Home) vs {away_team['name']} (Away) sudah pernah diselesaikan! Coba balik posisi Home/Away."
        }), 400

    # 2. If not finished, check if there is a scheduled/interrupted match we can resume
    ...
    if existing_unfinished:
        ...
        # First deactivate currently active matches
        ...
        db.matches.update_one(
            {'_id': existing_unfinished['_id']},
            {'$set': {'isLive': True, 'status': 'live'}}
        )
    ...
    # Deactivate any currently active match (handle both old and new schema)
    db.matches.update_many({'isLive': True}, {'$set': {'isLive': False, 'status': 'scheduled'}})
    db.matches.update_many({'status': 'live'}, {'$set': {'isLive': False, 'status': 'scheduled'}})

    # Check play order index
...
def update_match(current_user, match_id):
    ...
    if 'isLive' in data: update_fields['isLive'] = bool(data['isLive'])
    if 'isFinished' in data: update_fields['isFinished'] = bool(data['isFinished'])
    if 'isHidden' in data: update_fields['isHidden'] = bool(data['isHidden'])
...
def reset_match(current_user, match_id):
    ...
    # Check live status using both old and new schema
    ...
    if is_permanent or is_live:
        ...
        if is_live:
            ...
            log_action(current_user['username'], 'RESET_MATCH', f"Permanently reset/deleted match result: {match['team1']['name']} vs {match['team2']['name']} (Previous: {match.get('score1', 0)} - {match.get('score2', 0)}).")
    else:
        db.matches.update_one({'_id': ObjectId(match_id)}, {'$set': {'isHidden': True}})
        log_action(current_user['username'], 'HIDE_MATCH', f"Soft-deleted (hid) match: {match['team1']['name']} vs {match['team2']['name']} (Score was: {match.get('score1', 0)} - {match.get('score2', 0)}).")
